...
27.1 21:03-22:01 UTC 1SEC_NOISEFAC6.MDF NF6 -> data gap (for now)
27.1 22:01-23:03 UTC 1SEC_NOISEFAC3.MDF NF3 -> probably no data because of LV0 file size, data gap
...
30.1-31.1 23:42-11:37 UTC CU_SMALL_TINT_NF3_UNCOMPRESSED.MDF NF3 -> LV0 files are marked as corrupted
...
for begin_dt, end_dt in zip(begin_dates, end_dates):
    ...
    ds_name = f"{path}{begin_dt:%Y%m%d}_{time_frame}_LIMRAD94.nc"
    if not os.path.isfile(ds_name):
        ...
        check = input(f"{ds_name} already exists! \nDo you want to overwrite? "
                      f"(Warning! This action cannot be undone!) (y/n): ")
        if check.startswith("y"):
            print(f"Deleting {ds_name}...")
            os.remove(ds_name)
            std_above_mean_noise = 6.0
            LIMRAD_Zspec = build_extended_container(larda, 'VSpec', begin_dt, end_dt,
                                                    rm_precip_ghost=True,
...
print('Finished with all files.\nTotal elapsed time = {:.3f} sec.'.format(time.time() - start_time))