...
input_path = "Y:/data/"
output_path = "Y:/code/larda3/scripts/plots/PDFs_attitude_heave/"

# define date range, only works for one day at the time
begin_dt = datetime.datetime(2020, 2, 6, 0, 0, 0)
end_dt = datetime.datetime(2020, 2, 6, 23, 59, 59)
...
all_files = sorted(glob.glob(os.path.join(input_path + "DSHIP_RV-METEOR/*_10Hz.dat")))
file_list = []
for f in all_files:
...
arduino = pd.read_csv(f"{input_path}ARDUINO/final_daily_files/{begin_dt:%Y%m%d}_attitude_arduino.csv", sep=",",
                      usecols=[1, 2, 3, 4], index_col="datetime")
